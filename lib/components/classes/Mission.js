    ...
    this.processTurnActions = function() {
        if (this.isAllPlayersEndTurn()) {
            ...
            for (var startLocationId in allMoveActions) {
                for (var endLocationId in allMoveActions[startLocationId]) {
                    for (var mPlayerId in allMoveActions[startLocationId][endLocationId]) {
                        this.changeUnitsLocation(mPlayerId, startLocationId, endLocationId, allMoveActions[startLocationId][endLocationId][mPlayerId]);
            ...
            for (var i = 0; i < endLocationList.length; i++) {
                ...
                if (Object.keys(this.info.map[locationId].units).length != 0) {
...
//                    }
    ...
    this.changeUnitsLocation = function(mPlayerId, startLocationId, endLocationId, units) {
        for (var unitId in units) {
            this.info.map[startLocationId].units[mPlayerId][unitId] -= units[unitId];
            if (this.info.map[startLocationId].units[mPlayerId][unitId] == 0) {
                delete this.info.map[startLocationId].units[mPlayerId][unitId];
            }
        }
        if (Object.keys(this.info.map[startLocationId].units[mPlayerId]).length == 0) {
            delete this.info.map[startLocationId].units[mPlayerId];
        }
        
        if (this.info.map[endLocationId].units[mPlayerId] == null) {
            this.info.map[endLocationId].units[mPlayerId] = {};
        }
        
        for (var unitId in units) {
            if (this.info.map[endLocationId].units[mPlayerId][unitId] == null) {
                this.info.map[endLocationId].units[mPlayerId][unitId] = units[unitId];
            } else {
                this.info.map[endLocationId].units[mPlayerId][unitId] += units[unitId];
            }
        }
    ...
    this.getAllUnits = function() {
        ...
        for (var locationId in this.info.map) {
            if (Object.keys(this.info.map[locationId].units).length != 0) {
                units[locationId] = this.info.map[locationId].units;
            }
        }
        return units;
    };
